...
class BlockDetails extends Component {
  ...
  render () {
    ...
    if (hasError) {
      return <ErrorModal subject='Something went wrong' onClose={clearError}>Block details could not be loaded.</ErrorModal>
    }
    if (!loading && !block) {
      return <div>{ this.state.cid } : { this.state.errMsg }</div>
    }
    return (
      <div>
        {block ? (
          <div>
            <div>
              ...
              <div className='dib relative ml4' style={{height: 200, width: 100}}>
                {children && children.length ? (
                  <Link to={`/blocks/${children[0].cid}${hash}`} className='db absolute top-0 ph2 pv1 charcoal bg-snow br1 f6 link focus-outline' title='Newer'>
            ...
            {block.messages && block.messages.length ? (
              <section className='pv3'>
                <h2 className='f5 fw3 ttu tracked teal'>
                  Message Receipts <Badge>{block.messageReceipts ? block.messageReceipts.length : 0}</Badge>
                </h2>
